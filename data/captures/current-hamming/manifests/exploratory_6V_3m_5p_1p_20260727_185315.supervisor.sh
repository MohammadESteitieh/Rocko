...
RUN=exploratory_6V_3m_5p_1p_20260727_185315
...
tell application "System Events"
    activate
    display dialog "The live decoder is running. Confirm the lab is clear, click Start, and leave the transmitter area. The first frame starts after 15 seconds off." buttons {"Cancel", "Start test"} default button "Start test" cancel button "Cancel" with title "Rocko exploratory test"
end tell
APPLESCRIPT
printf 'operator_confirmed_lab_clear_utc=%s\n' "$(utc)" >> "$META"
/usr/bin/osascript -e 'display notification "Test started. Leave now; first signal begins after 15 seconds off." with title "Rocko exploratory test"' >/dev/null 2>&1 || true
