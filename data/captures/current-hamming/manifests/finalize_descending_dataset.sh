...
rpid=$(cat "$BASE/manifests/dataset_receiver.pid")
...
# Dataset is exactly 2115 s; leave additional clean H0 time and scheduling margin.
sleep 2175
status=$("${SSH[@]}" "cd /data/home/qnxuser/transmitter && if grep -q 'DIAGNOSTIC COMPLETE' '$remote_log' 2>/dev/null; then echo COMPLETE; elif kill -0 '$remote_pid' 2>/dev/null; then echo STILL_RUNNING; else echo FAILED; fi" 2>/dev/null) || {
  echo "NETWORK_ISSUE $(date -u +%FT%TZ)" > "$state"
  exit 2
}
if [ "$status" != COMPLETE ]; then
  echo "$status $(date -u +%FT%TZ)" > "$state"
  exit 3
fi
sleep 30
